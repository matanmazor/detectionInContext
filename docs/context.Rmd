...
In Exp. 1, target-present responses in the 90% quantile were faster by `r E1.quantile_congruency_effects %>%filter(probs==0.90)%>%pull(present)%>%mean()%>%abs()%>%round()` ms. in the congruent condition, but this effect was not significant (`r E1.quantile_congruency_effects %>%filter(probs==0.90)%>%pull(present)%>%t.test()%>%apa_print()%>%'$'(statistic)`). In Exp. 2, a difference of `r E2.quantile_congruency_effects %>%filter(probs==0.90)%>%pull(present)%>%mean()%>%abs()%>%round()` ms. in the same contrast reached significance (`r E2.quantile_congruency_effects %>%filter(probs==0.90)%>%pull(present)%>%t.test()%>%apa_print()%>%'$'(full_result)`). Similarly, a congruency effect of `r E2.quantile_congruency_effects %>%filter(probs==0.95)%>%pull(present)%>%mean()%>%abs()%>%round()` ms. was observed for 95% quantile target-present response times (`r E2.quantile_congruency_effects %>%filter(probs==0.95)%>%pull(present)%>%t.test()%>%apa_print()%>%'$'(full_result)`). Therefore, the absence of a context effct in target-absent trials cannot be explained by subjects adjusting their target-absent decisions times according to the slowest target-present responses.
...
### A two-stages account

Let's consider a very simple model for Exp. 1. In this model participants sample the stimulus once every timepoint and get a binary outcome: 0 or 1. When a target is absent, the outcome is always 0. But when a target is present, the outcome will start at 0 for the first samples but then become 1 at some other random point: the time of detection $\tau$, which is sampled from a Poisson distribution with parameter $\lambda$.

The subject's task is to decide when they have enough information to make a decision. After seeing 1, the decision is easy: they should terminate immediately and report target presence. The more difficult question is when to terminate when all samples are 0, because, theoretically, a positive sample may take forever to appear. One solution is to track the (log) likelihood ratio between target presence and absence $LLR=log\frac{p(x|Present)}{p(x|Absent)}$, and terminate once this quantity reaches some threshold, for example $LLR\leq-2$. Since $p(0|Absent)=1$, this quantity equals $log(p(x|Present))$, which is the log density of the $\tau$ distribution to the right of the current timepoint. This results in a rule: for every possible threshold on the LLR $X$, the agent should terminate after $T$ timesteps, where $T$ is the lowest timepoint for which which $LLR\leq X$.

(This model is cannot account for false positives, which constitute about 20% of our target-present trials, but it is a starting point).

One way to model the congruency manipulation, is by a change to the $\lambda$ parameter, which makes the first sample of 1 appear faster when a target is present. If that is the case, participants' failure to adjust their target-absent decision times reflects ignorance of the difference in the $\lambda$ values between congruent and incongruent conditions. An alternative is that the $\lambda$ parameter is unchanged, and instead, on some small proportion of trials in which the context is incongruent and the target is present, the target is perceived immediately. For example, on 15% of all such trials the first sample is 1, and all other 85% trials follow the same distribution as incongruent trials. Importantly, this reflects a change to the distribution of target detection times, moving more of its mass toward 0, and making long sequences of zeros less likely when a target is present. Subjects should still update their target-absent decision times if they want to use a similar criterion in both conditions.

A similar account may be more plausible in the case of Exp. 3. For example, on a certain proportion of trials in which the target is a 3 in the distractors are letters, the target may immediately pop out to attention (first parallel stage). On all other trials, searching for a 3 or an E among letters is just as efficient, and requires serial scanning. If participants' only decision is about whether they should serially scan the display or not, deciding not to serially scan on 3-in-letters searches will result in missing the targets on all trials in which the target did not pop out to attention. This is not the case if subjects have control over the number of items they want to attend to before terminating.

### Fixed criterion

An alternative explanation is that subjects are unable to dynamically adapt their decision criterion between trials within the same block, regardless of whether they are aware of differences in visibility or not. For example, @gorea2000failure found that subjects were unable to optimally set their decision criterion in a detection task involving two stimulus categories of low and high contrast, even when subjects were informed about the differences in visibility levels. Participants' false alarm rates were best accounted for by a model in which subjects marginalize over stimulus categories in extracting a likelihood ratio for making a decision.

It is worth noting that in other contexts, subjects have been shown to dynamically adjust their decision criterion between trials according to expected visibility. For example, @mazor2022efficient found that subjects were able to adaptively adjust their search times according to the expected difficulty of visual search trials even when presented within the same block. One possibility, however, is that what is possible for a visual search task is not possible for a near-threshold detection task. According to this account, subjects do know to expect that the perception of a letter would be easier in a congruent context, but they are unable to apply different decision thredholds within the same block.

### Speed-accuracy trade-off

Maybe subjects are aware of these differences in expected visibility between the two conditions, but they decide to enjoy the accuracy boost afforded by a congruent context without adjusting their decision times to equate accuracy between the two conditions. For example, a subject may know that 90% of all incongruent and 95% of all congruent targets would be detected within 2750 ms, and decide to set their decision criterion to 2750 and miss 10% of all incongruent and 5% of all congruent targets. This can be tested using a forced-stop paradigm.
