...
E1.frames_df <- read_csv('../experiments/letters/data/jatos_resultfiles_batch1/all_data.csv') %>%
...
E2.frames_df <- read_csv('../experiments/letters2/data/jatos_resultfiles_batch1/all_data.csv') %>%
  mutate(subj_id=PROLIFIC_PID,
         correct = as.numeric(correct),
         RT = as.numeric(RT),
         present=as.numeric(present),
...
E3.frames_df <- read_csv('../experiments/letters3occluded/version2/data/jatos_resultfiles_batch1/all_data.csv') %>%
  mutate(subj_id=PROLIFIC_PID,
         correct = as.numeric(correct),
         RT = as.numeric(RT),
         present=as.numeric(present),
...
Plotting the correlation between the presented frame as the target as a function of target presence.
...
E1.frames_df %>%
  ...
  summarise(r=mean(correlation_with_target_letter),
            se = se(correlation_with_target_letter)) %>%
  ...
  labs(x = 'frame index', y = 'correlation', title='Exp. 1') +
  theme_classic()
...
E2.frames_df %>%
  ...
  geom_line(size=1) + 
  geom_ribbon(aes(ymin=r-se, ymax=r+se, fill=present), alpha=0.5) +
  labs(x = 'frame index', y = 'correlation', title='Exp. 2') +
  theme_classic()
...
ggsave('../docs/figures/RC/visibility_by_presence_E3.png', width=4,height=3)
...
E1.frames_df <- E1.frames_df %>%
  filter(test_part %in% c('test1','test2'))%>%
  group_by(present,max_p,frame_index)%>%
  mutate(correlation_with_target_letter_corrected = 
           correlation_with_target_letter-mean(correlation_with_target_letter),
         present = factor(present, levels=c(0,1), labels=c('absent','present')))

E1.frames_df %>%
  group_by(present,frame_index)%>%
  summarise(r=mean(correlation_with_target_letter_corrected),
            se = se(correlation_with_target_letter_corrected)) %>%
  filter(frame_index<100) %>%
  ggplot(aes(x=frame_index,y=r,color=present)) +
  geom_line(size=1) + 
  geom_ribbon(aes(ymin=r-se, ymax=r+se, fill=present), alpha=0.5) +
...
E1.frames_df %>%
  ...
  group_by(subj_id,frame_index,cong) %>%
  summarise(decision=mean(correlation_with_target_letter_corrected[resp]-
                            correlation_with_target_letter_corrected[!resp])) %>%
  group_by(frame_index,cong) %>%
  summarise(se = se(decision, na.rm=T),
            decision=mean(decision, na.rm=T)) %>%
  ggplot(aes(x=frame_index,y=decision,ymin=decision-se,ymax=decision+se, color=cong, fill=cong)) +
  geom_abline(intercept=0,slope=0, color='black')+
  geom_line(size=1) +
  geom_ribbon(alpha=0.5) +
  labs(x='frame index', y='decision contrast',title='Exp.1')+
  theme_classic()
...
E2.frames_df %>%
  ...
  geom_line(size=1) +
  geom_ribbon(alpha=0.5) +
  labs(x='frame index', y='decision contrast',title='Exp.2')+
  theme_classic()
...
ggsave('../docs/figures/RC/decision_kernel_E3.png', width=4,height=3)
